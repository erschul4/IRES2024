...
df.micro<- "https://www.dropbox.com/scl/fi/by91liv1k8k8ozdhbidf1/micro_abundance-1.xlsx?rlkey=kwk3di319aw8rzhchpbfg9kg5&st=8drk2xs4&dl=1"
...
df.baerdecay<- "https://www.dropbox.com/scl/fi/m84p2hcurd8ccwv7hedve/baer_decay_calculated.xlsx?rlkey=1c2qfijkb5ng0mhngz2589ffe&st=m5chihu7&dl=1"
...
df.micro <- df.micro %>%
  mutate(
    ...
    Glycol         = factor(Glycol, levels = c("Y", "N")),nematodepercentage = suppressWarnings(as.numeric(nematodepercentage)))
...
#box plot/violin of nematode total abundance percentage and glycol
ggplot(df.micro, aes(x = Glycol, y = nematodepercentage, fill = Glycol)) +
  geom_violin(trim = FALSE, alpha = 0.25) +
  geom_boxplot(width = 0.18, outlier.shape = NA) +
  geom_jitter(width = 0.12, alpha = 0.25, size = 1) +
  labs(x = "Glycol Presence", y = "Nematode %", title = "Nematode total abundance % by glycol presence") +coord_cartesian(ylim = c(0, 275)) + theme_minimal() + guides(fill = "none")
